...
//        createTailCell(data.size()+4,headNames.length);
    ...
    private int createHeadCell( String[] headNames, int colWidths[]) {
        ...
        createRow(titleStyle,0x549,"****学院统计报表",0,0,0,headNames.length-1);
        //第二行
        HSSFCellStyle unitStyle = getStyle(12,1,true,false);
        createRow(unitStyle,0x190,"制作单位: ****学院ThoughtWorks工作室",1,1,0,headNames.length-1);
    ...
    public static void main(String[] args) throws IOException {
        ...
        File f = new File("/home/ubuntu/Desktop/aa.xls");
        if (f.exists())
            f.delete();
        f.createNewFile();
